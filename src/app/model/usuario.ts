...
export class Usuario extends Persona {
  ...
  public static buscarUsuarioValido(cuenta: string, password: string): Usuario | undefined {
    return Usuario.getListaUsuarios().find(usu => usu.cuenta === cuenta && usu.password === password);
  }
  ...
  public validarUsuario(): string {
    ...
    if (!usu) return 'El correo no es valido.';
    return '';
  }
  ...
  public validarCorreoPregunta(): string {
  ...
  recibirDatos(activatedRoute: ActivatedRoute, router: Router) {
    activatedRoute.queryParams.subscribe(() => {
      const nav = router.getCurrentNavigation();
      if (nav) {
        if (nav.extras.state) {
